                                    ...
                                    <div class="product-detail">
                                        ...
                                        @auth('client')
                                            <div class="add-to-cart-box bg-white">
                                                <button class="btn btn-add-cart addcart-button"    wire:click.prevent='addtocart({{ $product->id }})'>Add
                                                    <span class="add-icon bg-light-orange">
                                                        <i class="fa-solid fa-plus"></i>
                                                    </span>
                                                </button>
                                                @if ($product->cart->count() > 0)
                                                    <div class="cart_qty qty-box "@if ($product->cart->count() > 0) style="display:block !important;" @endif>
                                                        <div class="input-group">
                                                            <button type="button" class="qty-left-minus" wire:click.prevent='qtydecrement({{ $product->id }})'>
                                                                <i class="fa fa-minus" aria-hidden="true"></i>
                                                            </button>
                                                            <input class="form-control input-number qty-input" type="text" name="quantity" value="{{ $product->cart->sum('qty') }}">
                                                            <button type="button" wire:click.prevent='qtyincrement({{ $product->id }})' class="qty-right-plus" >
                                    ...
                                    <div class="product-detail position-relative">
                                        ...
                                        @auth('client')
                                            <div class="add-to-cart-btn-2 addtocart_btn">
                                                ...
                                                @if ($product->cart->count() > 0)
                                                    <div @if ($product->cart->count() > 0) style="display:block !important;" @endif
                                                        class="cart_qty qty-box-2">
                                                        <div class="input-group">
